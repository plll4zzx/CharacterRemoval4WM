...
def test_rand_attack(
    ...
    if atk_style=='token_fq':
        token_fq_name='_'.join([
            'TokenFq',wm_name, 
            dataset_name.replace('/','_'), 
            llm_name.replace('/','_'), 
        ])+'.json'
        rand_attack.token_fq_path=os.path.join('saved_data', token_fq_name)
        rand_attack.log_info(['token_fq_path:', rand_attack.token_fq_path])
