...
from MarkLLM.utils.utils import create_directory_for_file, load_config_file
...
class SIRUtils:
    ...
    def __init__(self, config: SIRConfig, *args, **kwargs) -> None:
        ...
        """
        self.config = config
        self.transform_model = self._get_transform_model('MarkLLM/'+self.config.transform_model_name, config.transform_model_input_dim).to(self.config.device)
        self.embedding_tokenizer = BertTokenizer.from_pretrained('MarkLLM/'+self.config.embedding_model_path)
        self.embedding_model = BertModel.from_pretrained('MarkLLM/'+self.config.embedding_model_path).to(self.config.device)
        self.mapping = self._get_mapping('MarkLLM/'+self.config.mapping_name)
